...
ALTER COLUMN statusName VARCHAR(20) NOT NULL
...
BEGIN
	...
	SELECT @dTitle=(SELECT d.designationTitle FROM tbl_designation d WHERE d.designationID=IDENT_CURRENT('tbl_designation'))
END
GO
...
DECLARE @dTitle VARCHAR(MAX)
EXEC sp_insertDesignation 1,'Manager',50000, @dTitle OUTPUT
SELECT @dTitle 'Designation Name'
EXEC sp_insertDesignation 2,'Assistant Manager',40000, @dTitle OUTPUT
SELECT @dTitle 'Designation Name'
EXEC sp_insertDesignation 2,'Trainee Programmer',20000, @dTitle OUTPUT
SELECT @dTitle 'Designation Name'
EXEC sp_insertDesignation 1,'Cleaner',10000, @dTitle OUTPUT
SELECT @dTitle 'Designation Name'
EXEC sp_insertDesignation 3,'Driver',10000, @dTitle OUTPUT
SELECT @dTitle 'Designation Name'
GO
