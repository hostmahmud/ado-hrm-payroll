    ...
    public partial class frmDesignation : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (ValidateChildren(ValidationConstraints.Enabled))
            {
                try
                {
                    ...
                    {
                        cmd.CommandText = "UPDATE tbl_designation SET name='"+ad.Name+"',deptId="+ad.DeptId+",salaryAmount="+ad.Salary+",status="+ad.Status+" WHERE id = "+ad.Id+"";
                        if (cmd.ExecuteNonQuery() > 0)
                        {
                            MessageBox.Show("Designation ID " + ad.Id + " has been updated!", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            btnSave.Text = "Save";
                    ...
                    {
                        cmd.CommandText = "insert into tbl_designation values('" + ad.Name + "'," + ad.DeptId + "," + ad.Salary + "," + ad.Status + ")";
                        if (cmd.ExecuteNonQuery() > 0)
                        {
                            MessageBox.Show("Designation ID " + ad.Id + " has been saved!", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show("Something wrong");
        ...
        private void txtName_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!(char.IsLetter(e.KeyChar) || e.KeyChar == (char)Keys.Back))
            {
                e.Handled = true;
                txtName.BackColor = Color.Red;
                errorProvider1.SetError(txtSalary, "Only A to Z allowed");
            }
            else
            {
                e.Handled = false;
